...
public class DependentIntegrationTests : IntegrationTest
{
    ...
    public async Task WhenAskedForADependent_ShouldReturnCorrectDependent()
    {
        ...
        // Act
        var result = _dependentService?.GetDependentById(dependent.Id);

        // Assert
        Assert.NotNull(result);
        Assert.Equal(dependent.Id, result?.Id);
        Assert.Equal(dependent.FirstName, result?.FirstName);
        Assert.Equal(dependent.LastName, result?.LastName);
    }
